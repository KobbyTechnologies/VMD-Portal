...
class productDetails(UserObjectMixin, View):
    def get(self, request, pk):
        try:
            ...
            CountriesRegister = [
                x for x in CountriesRegisteredResponse['value']]
            ...
            MarketingAuthorisationResponse = self.get_object(
    ...
    if request.method == 'POST':
        try:
            ...
            DateAuthorisation = datetime.strptime(
                request.POST.get('DateAuthorisation'), '%Y-%m-%d').date()
    ...
    def post(self, request, pk):
        if request.method == 'POST':
            try:
                ...
                response = config.CLIENT.service.FnRegistrationPayment(
                    prodNo, userCode)
